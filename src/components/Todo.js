...
function Todo() {
    ...
    const saveEditedTask = (id) => {
        ...
        axios.put('http://127.0.0.1:3001/updateTodoList/' + id, editedData)
            .then(result => {
                setTodoList(todoList.map(todo => (todo._id === id ? result.data : todo)));
                setEditableId(null);
                setEditedTask("");
                setEditedStatus("");
                setEditedDeadline("");
    ...
    const deleteTask = (id) => {
        axios.delete('http://127.0.0.1:3001/deleteTodoList/' + id)
            .then(result => {
                ...
                setError(null); // Clear error if deletion is successful
            })
